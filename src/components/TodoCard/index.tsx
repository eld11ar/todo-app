...
const TodoCard: FC<Props> = ({ todo }) => {
	...
	const isTodoDatePassed =
		todoDate >= today
			? `${styles.datePassed} ${styles.wrapper}`
			: styles.wrapper;
	const isBtnDisabled =
		todoDate >= today
			? `${styles.dateBtnPassed} ${styles.dateButton}`
			: styles.dateButton;
	...
	return (
		<>
			<div onClick={handleOpenModal} className={isTodoDatePassed}>
				...
				<Spoiler openModal={handleOpenModal}>{todo.description}</Spoiler>
